...
class UploadHandler
{
    ...
    function __construct($options=null) {
        $this->options = array(
			...
			'upload_dir' => '/opt/dm/ajaxplorer/data/personal/',
			'upload_url' => '/opt/dm/ajaxplorer/data/personal/',
            'param_name' => 'files',
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error) {
		...
		if (isset($_REQUEST['subdir'])){
			$folder=$_REQUEST['subdir'];
			if(!is_dir($this->options['upload_dir'].$folder)){
				mkdir($this->options['upload_dir'].$folder);
			}
		}
        $file = new stdClass();
        ...
        if (!$error && $file->name) {
			...
			//error_log('il nostro file = '.$file_path);
            $append_file = !$this->options['discard_aborted_uploads'] &&
                is_file($file_path) && $file->size > filesize($file_path);
            clearstatcache();
			//error_log('result = '.$append_file);
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
            ...
            if ($file_size === $file->size) {
				...
				error_log("########## CHIAMA QUI SPOSTAMENTENTO in GRID ###########");
